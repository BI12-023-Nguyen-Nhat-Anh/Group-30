    ...
    if intended_use == "HouseHold":
        ...
        if (name == "") or (email == "") or (province == "") or (district == "") or (ward == "") or (residental_adress == "") or (intended_use == ""):
            ...
            if formula_email(email) == False:
                ...
                new_data = {'Customer Code': f'CH{str(random_number)}','Name':name_cus,'Electricity usage address': electricity_usage,'Residential address': address,'Phone Number': phone,'Email': mail,'Identify number': ident,'Tax Code':'None','Type': type_cus }
                ...
                new_data_1 = {'Customer Code': f'CH{str(random_number)}','Name': name_cus,'Electricity usage address': electricity_usage,'Phone Number': phone,'Identify number': ident,'Tax Code': 'None','Type': type_cus}
        ...
        if (name == "") or (email == "") or (province == "") or (district == "") or (ward == "") or (residental_adress == "") or (intended_use == "") or (tax == ""):
            ...
            if formula_email(email) == False:
                ...
                new_data = {'Customer Code': f'CH{str(random_number)}','Name':name_cus,'Electricity usage address': electricity_usage,'Residential address': address,'Phone Number': phone,'Email': mail,'Identify number': ident,'Tax Code':tax,'Type': type_cus}
                df_new = pd.DataFrame(new_data,index=[0])
                updated_df = pd.concat([df,df_new], ignore_index=True)
                updated_df.to_excel(writer, sheet_name='data_customer', index=False)

                new_data_1 = {'Customer Code': f'CH{str(random_number)}','Name': name_cus,'Electricity usage address': electricity_usage,'Phone Number': phone,'Identify number': ident,'Tax Code': tax,'Type': type_cus}
                df1_new = pd.DataFrame(new_data_1,index=[0])
                updated_df_1 = pd.concat([df1,df1_new], ignore_index=True)
                updated_df_1.to_excel(writer, sheet_name='filtered_data', index=False)
                window.destroy()
